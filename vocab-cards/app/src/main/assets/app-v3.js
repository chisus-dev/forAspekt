(() => {
  ...
  function defaultSrs() {
    return {
      due: Date.now(), intervalDays: 0, ease: 2.25, reps: 0,
      lapses: 0, lastGrade: null, lastReviewedAt: null
    };
  }
  ...
  function sanitizeCard(card) {
    ...
    const translations = (Array.isArray(card.translations) ? card.translations : [])
      .map(item => typeof item === 'string'
        ...
        : { id: item.id || uid('tr'), text: String(item.text || '').trim(), srs: sanitizeSrs(item.srs) })
    ...
    const examples = (Array.isArray(card.examples) ? card.examples : [])
      .map(example => ({
        ...
        text: String(example.text || '').trim(),
        translationId: example.translationId || null,
  ...
  function loadCards() {
    try {
      ...
      if (Array.isArray(current)) return current.map(sanitizeCard).filter(Boolean);
    } catch (_) {}
    try {
      const legacy = JSON.parse(localStorage.getItem(LEGACY_KEY) || 'null');
      if (Array.isArray(legacy)) {
        const migrated = legacy.map(migrateLegacyCard).filter(Boolean);
  ...
  function directExamplesForTranslation(card, translationId) {
    return getExamplesForCard(card).filter(example =>
      example.ownerCardId === card.id && example.translationId === translationId
    );
  }
  ...
  function countDueTargets() {
    ...
    state.cards.forEach(card => {
      ...
      card.translations.forEach(item => { if (item.srs.due <= now) count += 1; });
    });
    return count;
  }
  ...
  function renderCards() {
    ...
    const cards = state.cards
      .filter(card => !query || normalizeAnswer([card.word, ...card.translations.map(item => item.text)].join(' ')).includes(query))
  ...
  function refreshExampleTranslationOptions() {
    ...
    els.exampleList.querySelectorAll('.example-editor').forEach(node => {
      const select = node.querySelector('.example-translation');
      const selected = select.value || node.dataset.translationId || '';
      select.innerHTML = '<option value="">Без привязки к значению</option>' + translations
        .map((item, index) => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.text || `Перевод ${index + 1}`)}</option>`)
        .join('');
      select.value = translations.some(item => item.id === selected) ? selected : '';
      node.dataset.translationId = '';
    });
  }
  ...
  function renderDetectedLinks(exampleNode) {
    ...
    const matches = text ? state.cards.filter(card => card.id !== currentId && sentenceContainsWord(text, card.word)) : [];
  ...
  function collectExampleDrafts(validTranslationIds) {
    return Array.from(els.exampleList.querySelectorAll('.example-editor'))
      .map(node => {
        const translationId = node.querySelector('.example-translation').value;
        return {
          id: node.dataset.exampleId || uid('ex'),
          text: node.querySelector('.example-input').value.trim(),
          translationId: validTranslationIds.has(translationId) ? translationId : null,
          linkedCardIds: Array.from(new Set(Array.from(node.querySelectorAll('.detected-links input:checked')).map(input => input.value)))
        };
      })
      .filter(item => item.text);
  }
  ...
  function openModal({ title, text, actions }) {
    ...
    return new Promise(resolve => {
      ...
      els.modalActions.querySelectorAll('[data-modal-index]').forEach(button => {
        button.addEventListener('click', () => state.modalResolver(actions[Number(button.dataset.modalIndex)].value));
      });
    });
  }
  ...
  function chooseTask(options = {}) {
    ...
    let tasks = remediation.length
      ? remediation
      : buildTasks({ includeFuture: state.practiceAnyway || options.force, onlyCardId: options.onlyCardId });
  ...
  function materializeTask(task) {
    ...
    if (type === 'reverse') examples = getExamplesForCard(card);
    if (type === 'all') examples = getExamplesForCard(card);
    const example = examples && examples.length ? examples[Math.floor(Math.random() * examples.length)] : null;
  ...
  function renderStudyHome() {
    ...
    const nextDue = Math.min(...state.cards.flatMap(card => [card.srs.due, ...card.translations.map(item => item.srs.due)]));
  ...
  function renderStudyQuestion() {
    ...
    if (task.type === 'all') {
      ...
      context = getExamplesForCard(task.card).slice(0, 3).map(example => `<div class="context-box">${formatContext(example.text, task.card.word)}</div>`).join('');
      ...
      help = 'Укажите именно значение, привязанное к этому предложению.';
    } else {
      ...
      context = task.example ? `<div class="context-box">${formatContext(task.example.text, task.card.word, 'mask')}</div>` : '';
      placeholder = 'English word';
      help = 'Регистр не учитывается. Опечатки автоматически не исправляются.';
    }
  ...
  function evaluateCurrentAnswer() {
    ...
    if (task.type === 'all') {
      ...
      const rememberedIds = [];
      const missedIds = [];
      const expectedNormalized = new Map(task.card.translations.map(item => [normalizeAnswer(item.text), item.id]));
      ...
      const expected = task.type === 'reverse' ? task.card.word : task.translation.text;
  ...
  function applyGradeToCopy(srsValue, grade, now) {
    ...
    if (grade === 'again') {
      ...
      srs.intervalDays = srs.intervalDays < 1 ? .33 : Math.min(120, Math.max(1, srs.intervalDays * 1.45));
      srs.due = now + srs.intervalDays * DAY;
    } else {
      srs.ease = Math.min(3, srs.ease + .04);
      srs.intervalDays = srs.intervalDays < 1 ? 1 : Math.min(365, Math.max(2, srs.intervalDays * srs.ease));
      srs.due = now + srs.intervalDays * DAY;
    }
    return srs;
  }
  ...
  function scheduleRemediation(cardId, translationId, preferredType) {
    if (state.remediationQueue.some(item => item.cardId === cardId && item.targetId === translationId)) return;
    const card = getCard(cardId);
    ...
    if (updateSchedule) {
      if (task.type === 'all') {
        ...
        task.card.translations.forEach(translation => {
          const translationGrade = overrideGrade || (evaluation.rememberedIds.includes(translation.id) ? 'good' : 'again');
  ...
  function handleEditorClick(event) {
    ...
    if (action === 'remove-example') {
      ...
      if (els.exampleList.querySelectorAll('.example-editor').length === 1) {
        ...
        row.querySelector('.example-translation').value = '';
